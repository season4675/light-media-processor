...
int ReadBuffer(char *requested_buffer, int requested_len, char **src_buffer,
               int *src_bytes, int sample_bytes) {
  if (*src_bytes == 0 || NULL == *src_buffer) return 0;

  // input length is smaller than the length requested
  if (*src_bytes < requested_len) {
    memcpy(requested_buffer, *src_buffer, *src_bytes);
    requested_len = *src_bytes;
    *src_bytes = 0;
    free(*src_buffer);
    *src_buffer = NULL;
    return requested_len / sample_bytes;
  }

  memcpy(requested_buffer, *src_buffer, requested_len);

  // save the left data to tmp buffer
  *src_bytes = *src_bytes - requested_len;
  if (*src_bytes > 0) {
    char *tmp_buffer = (char *)malloc(*src_bytes);
    memcpy(tmp_buffer, *src_buffer + requested_len, *src_bytes);
    free(*src_buffer);
...
int64_t WavRead(void *read_info, float *buffer, int samples, char **inbuf,
  ...
  requested_sample_num =
      ReadBuffer(reinterpret_cast<char *>(requested_buf), requested_len, inbuf,
                 inbuf_bytes, sample_bytes);
  wav_info->samplesread += requested_sample_num;

  int requested_1ch_sample_num = requested_sample_num / wav_info->channels;

  for (int i = 0; i < requested_1ch_sample_num; i++) {
...
static int64_t ReadPadder(void *read_info, float *buffer, int samples,
                          char **inbuf, int *inbuf_bytes) {
  Padder *padder = reinterpret_cast<Padder *>(read_info);
  // use WavRead Func here
  int64_t in_samples =
      padder->read_func(padder->read_info, buffer, samples, inbuf, inbuf_bytes);
  if (in_samples <= 0) {
